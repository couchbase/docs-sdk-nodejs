...
async function insertHandler(request, h) {
    const key = request.query.k ? request.query.k : docKey;
    [lastDocument.type, lastDocument.id] = key.split("_");
    try {
        // #tag::insert[]
        const result = await collection.insert(key, lastDocument);
        // #end::insert[]
        lastCAS = result.cas;
        return h.response(result);
    } catch (e) {
...
async function durabilityHandler(request, h) {
    ...
    try {
        // #tag::durability[]
        let result = await collection.upsert(key, lastDocument, {
            cas: lastCAS,
            expiration: 60, // 60 seconds
            persistTo: 1,
            replicateTo: 1,
...
async function removeHandler(request, h) {
    // if lastCAS is from some other lastDocument, this will fail.
    // is failing a good example of cas?  Or should we
    // 'get' the lastDocument to get the latest cas?
    const key = request.query.k ? request.query.k : docKey;
    console.log("remove cas: " + lastCAS + " key: " + key);
    try {
        // #tag::remove[]
...
async function incrementHandler(request, h) {
    ...
    const result = await collection.binary().increment(binValKey, 1, null, null);
    // #end::increment[]
    return h.response(result);
}
...
async function incrementwithoptionsHandler(request, h) {
    try {
        ...
        // increment binary value by 1, if lastDocument doesn’t exist, seed it at 1000
        const result = await collection.binary().increment(
            binValKey,
            1,
            {
                initial: 0,
...
async function decrementwithoptionsHandler(request, h) {
    try {
        ...
        // decrement binary value by 1, if lastDocument doesn’t exist, seed it at 1000
        const result = await collection.binary().decrement(binValKey, 1, {
            timeout: 5000
        });
