...
You can flush a bucket in the SDK by using the https://docs.couchbase.com/sdk-api/couchbase-node-client/BucketManager.html#flushBucket[`flushBucket()` method]:
////
[source,golang,indent=0]
----
...
----
////
The `flushBucket()` operation may fail if the bucket does not have flush enabled, in that case it will return an `BucketNotFlushableError`.
