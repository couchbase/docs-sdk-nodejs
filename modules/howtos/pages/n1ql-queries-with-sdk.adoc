...
If you are performing an operation with N1QL that requires CAS to be used, in combination with using CAS from regular KV operations,
then you need to be aware of the xref:howtos/concurrent-document-mutations.adoc#cas-value-format[CAS type], and the possibility of node.js not being able to represent it.

Node.js, which handles the deserialization, has a limitation in the way it represents integers -- thus you must convert to a string:
