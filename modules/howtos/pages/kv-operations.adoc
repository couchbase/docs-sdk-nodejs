...
See xref:concept-docs:document-operations.adoc[the concept guide to _Documents_] for a deeper dive into documents in the Couchbase Data Platform.
...
In Couchbase Server releases before 6.5, Durability was set with these two options -- see the https://docs.couchbase.com/nodejs-sdk/2.6/durability.html[6.0 Durability documentation] -- covering  how many replicas the operation must be propagated to and how many persisted copies of the modified record must exist. 
Couchbase Data Platform 6.5 refines these two options, with xref:6.5@server:learn:data/durability.adoc[Synchronous Replication], allowing discrete control of replication and persisteence across a number of nodes before the mutation is available.
