...
Include Mozilla Root CAs by Default
...
** There are several improvements has been implemented to make the library resilient to rapid topology changes (in particular in Cloud environment) when both DNS-SRV bootstrap is being used along with alternative addresses. 
The changes include:
...
this way we will be sure that no duplicate connections will be left, or live connections replaced by restarted session.
