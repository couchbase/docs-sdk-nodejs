...
The Couchbase Node.js Client will run on any https://github.com/nodejs/Release[supported LTS version of Node.js] -- see the table <<supported-node-js-versions,below>>.
...
Couchbase Node.js SDK is supported on any Node.js LTS version that is:

. https://github.com/nodejs/Release[Supported] at time of SDK release (and thus tested).
. Has not reached https://endoflife.date/nodejs[End-of-Life date].
...
.Binary Size
====
You may find the binary of the SDK too large to run in some environments.
...
You can also safely remove the `src/` and `deps/` folders (under `couchbase/node_modules/`).
====
