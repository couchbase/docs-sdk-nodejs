...
:description: The 3.x API (used in Node.js SDK 3.x and 4.x)  breaks the previous 2.x APIs (used in Node.js SDK 2.x) in order to provide a number of improvements. \
Collections and Scopes are introduced.
:nav-title: Migrating to Node.js SDK 3.x API
...
Additionally, the Node.js SDK 4.0 offers a new backend (Couchbase++) with better support for upcoming features. Though the API still matches the SDK 3.x spec, we have increased the major version to reflect the importance of this implementation change, as per https://semver.org/[semantic versioning].

Individual behaviour changes across services are explained on this page.

NOTE: if you are an existing SDK 3.x user considering migrating to SDK 4.0,
...
As a migration guide, this document assumes you are familiar with the previous generation of the SDK and does not re-introduce SDK 2.x concepts.
...
The Node.js SDK 3.x and 4.x are available through `npm`, just like the previous generation.
Please see the xref:sdk-release-notes.adoc[Release Notes] for up-to-date information.


The Nodes.j SDK 3.x has the following dependencies:
...
Additionally, the Node.js SDK 4.0 offers a new backend (Couchbase++) with better support for upcoming features.
...
// view query reduce option string-> boolean // TODO word this more clearly
* Search scan consistency `request-plus` has been removed.
