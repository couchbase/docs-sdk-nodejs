...
////
+ The following is for Java SDK - Does this also apply to nodejs?
If no custom environment is passed in, the default settings can still be customized
through either the connection string or xref:ref:client-settings.adoc[system properties].
The SDK has elaborate reflection logic in place to parse "flat" string values and
apply them to the builder, which means that you can now configure more properties
than in SDK 2.
...
////
+ java only, not nodejs

[source,javascript]
----
include::devguide:example$nodejs/migrating.js[tag=sysprops,indent=0]
----
-
////
...
[source,javascript]
----
include::devguide:example$nodejs/migrating.js[tag=simpleget,indent=0]
----
...
// Table is for Java. I'm not sure if nodejs has same mappings as Java.
////
+
...
Each `CouchbaseErroreption` has an associated `ErrorContext` which is populated with as much info as possible and then dumped alongside the stack trace if an error happens.
...
Not only does it throw a `CouchbaseErroreption`, it also tries to map it to a specific exception type and include extensive contextual information for a better troubleshooting experience.

////
// surely this doesn't apply to Node??
==== DSL Deprecation
