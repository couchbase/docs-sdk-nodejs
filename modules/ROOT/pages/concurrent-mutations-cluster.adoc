...
[source,javascript]
----
function updateEmail(bucket, userId, newEmail, callback) {
    var tryUpdateEmail = function() {
        bucket.get(userId, function(err, res) {
            ...
            bucket.upsert(userId, userDoc, function(err, res) {
...
The following blocks show how to use [.api]`lock` and [.api]`unlock` with the Node.js SDK.

[source,javascript]
----
bucket.lock('key', function(err, res) {
    if (err && err.code === couchbase.errors.temporaryError) {
        // document already locked
        return;
    }

    var lockedCas = res.cas;

    /** an example of simply unlocking the document
    bucket.unlock('key', lockedCas, function(err, res) {
        // document is now unlocked
    })
    */

    // update the contents of the locked document and unlock it
    bucket.upsert('key', 'new value', {cas: lockedCas}, function(err, res) {
        // the document is now updated and unlocked.
    });
});
----
