...
You should use Sub-Document operations when you are modifying only portions of a document, and full-document operations when the contents of a document are to change significantly.
...
----
bucket
  .lookupIn('customer123')
  .get('addresses.delivery.country')
  .execute(function(err, res) {
    var country = res.content('addresses.delivery.country');
    // => 'United Kingdom'
  });
----
...
The simplest of these operations is _subdoc-upsert_, which, just like the fulldoc-level _upsert_, will either modify the value of an existing path or create it if it does not exist:
...
bucket
  .mutateIn('customer123')
  .insert('purchases.completed', [42, true, null])
  .execute(function(err, res) {
...
[source,javascript]
----
bucket.upsert('my_array', [], function(err, res) {
  bucket
    .mutateIn('my_array')
...
If you wish to create an array which does not exist and also push elements to it within the same operation you may use the _create-parents_ option:

[source,javascript]
----
bucket
  .mutateIn('some_doc')
  .arrayAppend('some.array', 'Hello', {createParents: true})
  .execute(function(err, res) {
    // ...
  });
----
...
bucket
  ...
  .counter('logins', 1)
  .execute(function(err, res) {
    var cur_count = res.content('logins');
    // cur_count => 1
  });
----
...
[source,javascript]
----
bucket.upsert('player432', {gold: 1000}, function(err, res) {
  bucket
    .mutateIn('player432')
    .counter('gold', -150)
    .execute(function(err, res) {
      // player 432 now has 850 gold
      var gold_remaining = res.content('gold');
      // gold_remaining => 850
    });
});
----
...
bucket
  .mutateIn('customer123')
  .upsert('level_0.level_1.foo.bar.phone', {num: '311-555-0101', ext: 16}, {createParents: true})
  .execute(function(err, res) {
    // ...
  });
----
...
In other words, the path component must exactly match the path inside the document itself.
For example:
...
[source,javascript]
----
function escape_component(component) {
  var componentJson = JSON.stringify(component);
  var component = componentJson.substr(1, componentJson.length - 2);
  return '`' + component + '`';
}
console.log(escape_component('Hello!')) // `Hello!`
console.log(escape_component('backtick[]')) // `backtick[]`
console.log(escape_component('["mixed\\') // `[\"mixed\\`
----
