...
[source,javascript]
----
upsert(docId, newValue, options, callback)
insert(docId, newValue, options, callback)
replace(docId, newValue, options, callback)
get(docId, options, callback)
remove(docId, options, callback)
----
...
[source,javascript]
----
bucket.lookupIn('user:kingarthur')
  .get('contact.name')
  .get('contact.email')
  .execute(function(err, res) {
    var name = res.contentByIndex(0);
    var email = res.contentByIndex(1);
})
----
...
[source,javascript]
----
bucket.counter('counter_id', {delta: 20, initial: 100}, function(err, res) {
    // counter is now 100

    bucket.counter('counter_id', {delta: 1}, function(err, res) {
        // counter is now 101

        bucket.counter('counter_id', {delta: -50}, function(err, res) {
            // counter is now 51
        });
    });
});
...
In the above example, a counter is created by using the `counter` with an `initial` value.
...
[source,javascript]
----
bucket.get('counter_id', function(err, res) {
    if (should_increment_value(res.value)) {
        var newValue = res.value + increment_amount;
        bucket.upsert('counter_id', newValue, {cas: res.cas}, function(err, res) {
            // updated
        });
    }
})
...
CAUTION: JavaScript does not support 64-bit integers (while libcouchbase and Couchbase Server do). You might receive an inaccurate value if the number is greater than 53-bits (JavaScript's maximum integer precision).
...
You will find several ways of working with counters https://docs.couchbase.com/sdk-api/couchbase-node-client-2.6.8/Bucket.html#counter__anchor[in the API docs].
////
...
See the xref:datastructures.adoc[data structures documentation for each SDK language] for details on implementation.
...
Data structures in Couchbase are similar in concept to the data structures found in JavaScript:

* *Map* is like the JavaScript `Map`, introduced in ES6.
Prior to this, it's like a JS`Object`, being a key-value structure, where a value is accessed by using a key string.
...
[source,javascript]
----
bucket.mapAdd('map_id', 'name', 'Mark Nunberg', {upsert: true}, function(err, res) {
    // item added to the map
})
----

Data structures can be accessed using their appropriate methods.
Most data access methods will return an object with the actual returned value under the [.var]`value` property.

[source,javascript]
----
bucket.mapGet('map_id', 'name', function(err, res) {
    var name = res.value; // Mark Nunberg
});
----
...
[source,javascript]
----
append(docId, fragment, options, callback)
prepend(docId, fragment, options, callback)
----
...
bucket.upsert('binary_doc', Buffer.from([1]), function(err, res) {
    
    // append some bytes to the document
    bucket.append('binary_doc', Buffer.from([2, 3, 4]), function(err, res) {

        // fetch the document
        bucket.get('binary_doc', function(err, res) {
            var binaryValue = res.value;

            console.log(binaryValue); // Buffer(1, 2, 3, 4)
        })
    });
});
----
...
NOTE: Although the API only sets expiry values _per document_, it is possible that elsewhere in the server, an expiry value is being set for xref:6.0@server:learn:buckets-memory-and-storage/expiration.adoc[every document in a bucket^].
