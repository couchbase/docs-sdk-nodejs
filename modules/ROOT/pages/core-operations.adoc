...
* *Queue* is a wrapper over an _array_ which offers FIFO (first-in-first-out) semantics, allowing it to be used as a lightweight job queue.
* *Set* is a wrapper over an _array_ which provides the ability to handle unique values.
