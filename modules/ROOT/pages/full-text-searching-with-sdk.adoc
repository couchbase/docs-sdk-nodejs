...
A location is represented by means of longitude-latitude coordinate pairs.
...
* A location, specified as a longitude-latitude coordinate pair; and a distance, in miles. The location determines the center of a circle whose radius-length is the specified distance.
Documents are returned if they reference a location within the circle
(this is the case in the example _below_).
* Two longitude-latitude coordinate pairs. These are respectively taken to indicate the top left and bottom right corners of a bounding box.
...
NOTE: In the current (2.6.7) Node.js SDK, _longitude_ must be given before _latitude_ for the `geoDistanceQuery` method.
This bug does not apply to the 3.0 version of the SDK.

// ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ check with Brett

// https://issues.couchbase.com/browse/JSCBC-589
...
​
// lon, lat, distance (with units, eg 100mi, 20km. Accepts (in|ft|yd|mi|mm|cm|m|km|nm) where nm is nautical miles)
...
// (and descending(true) cuts off the closest results.)
// (so most useful output is probably to reverse descending(false))
q.sort([
