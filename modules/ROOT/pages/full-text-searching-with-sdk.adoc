...
A general introduction to FTS, with pointers to detailed descriptions of its principal features, is provided in xref:5.1@server:fts:full-text-intro.adoc[Full Text Search: Fundamentals].
...
For information on installing sample buckets, see xref:5.1@server:settings:install-sample-buckets.adoc[Sample Buckets].
