...
On non-windows platforms you will also need to install https://github.com/nodejs/node-gyp[node-gyp^]. This can be installed with `npm`, and should also be available in your distribution's package manager:
...
Older versions of Ubuntu, for example Ubuntu 14.04, will not have a new enough version of Node.js. Updating or installing any software via a shell script from the Internet with CURL is not recommended. To install newer versions of Node.js follow these steps:
