...
This can be useful for manually debugging what's causing a specific error, or why a connection might be dropping etc.
However, it's not the best solution for simply checking the status of a node for example, due to the sheer volume of information it outputs.

xref:threshold-logging.adoc[Threshold Logging] will periodically output a list of operations that took longer than a specified timer to complete.
...
But the most useful tool for this context is the Health Check API.
If after a few timeouts or failures (remember, these can always be counted/tracked explicitly as part of the application code) we may suspect there's a problem server-side.
