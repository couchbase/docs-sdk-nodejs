= Tracing
:description: Tracing and Metrics provide fine-grained insight into how an application is performing, and helps to diagnose when it is not.
// :nav-title: Request Tracing and Metrics
:page-aliases: ROOT:threshold-logging.adoc
:page-toclevels: 2


[abstract]
{description}


[TIP] 
====
In addition to Tracing and other metrics, and client logging, SDK is telemetry is also sent to the Server -- available from 8.0, and in new Capella Operational clusters -- for ingestion with other Prometheus metrics.
Capella Operational exposes these metrics through the UI.
====



include::{version-common}@sdk:shared:partial$rto.adoc[tag=rto]


=== Performance Trade-Off

SDK observability is on by default.
The information it provides in invaluable when diagnosing network bottlenecks.
There may be use cases where disabling observability is desirable to slightly improve raw performance,
but be sure of your environment first, as well as the relative importance of absolute performance,
as it will be harder to diagnose network problems in production.
